...
A Worker starts only with a validated project configuration, an approved plan, exact Task IDs, a dedicated Worktree, a persisted Task record, and a complete dispatch package.

Confirm the plan in the Worker worktree before implementation. A missing, stale, or mismatched plan blocks execution and returns an actionable status to the Planner.

## Execution

1. Load every configured `local:` Worker context.
2. Apply `$m-execute` to only the approved Task IDs and write sets.
3. Preserve its input validation, error handling, file ownership, rollback, and lightweight-validation rules.
4. Compute a stable change identifier for the exact state being admitted to testing, such as the current commit plus a worktree diff hash.
5. Write a lightweight-gate evidence file that contains commands, exit statuses, applicability/skips, and the change identifier but no context secrets.

## Lightweight Gate

Run every applicable cheap check before Tester admission:
...
Any implementation edit after gate creation invalidates that gate. Repair work must produce a new change identifier and rerun the complete gate.
...
4. After a lease is issued, create one temporary Tester sub-agent in the same Worker Worktree with the complete stage `3.3` context package.
...
Persist the passing `$m-test` result, release Tester capacity, transition through `TEST_PASSED` to `WAITING_FOR_MERGE`, and enqueue the capacity-one integration pool. After admission, invoke `$m-archive` as the archive, merge, and cleanup authority.
