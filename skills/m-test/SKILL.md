...
Use this skill for heavyweight validation and review after `$m-execute`. It is optional: skip it for low-risk small changes when execution-stage checks already cover the change.
...
7. If any review item fails, record the failing item and return to `$m-execute`.
8. If skipped or all run items pass, report that the workflow may proceed to `$m-archive`.
